variable "env" {
  description = "Environment prefix used in the generated container group name"
  type        = string
  default     = "livetest"
}
...
variable "tags" {
  description = "Tags applied to the container group created by this harness"
  type        = map(string)
  default = {
...
variable "container_group" {
  description = "Container group configuration object, passed straight through to the module under test"
  type        = any
}
