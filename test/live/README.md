...
| `test_dependencies.tf` | A dedicated, throwaway resource group + vnet + subnet this harness owns outright - never a shared/production resource. The subnet carries the `Microsoft.ContainerInstance/containerGroups` delegation the module's default `ip_address_type = "Private"` requires. All names are suffixed with `var.pr_number` so concurrently open PRs never collide. |
| `variables.tf` | `env`, `location` (defaults to `canadacentral`), `tags`, `pr_number` (defaults to `"manual"`), and `container_group` (typed `any`, passed straight through to the module). |
| `config/container-groups.tfvars` | One representative real-usage fixture: a single public-image container on the private subnet, legacy single port/protocol keys. |
...
cd test/live
terraform init
terraform plan  -var-file=config/container-groups.tfvars
terraform apply -var-file=config/container-groups.tfvars
```

Confirm only the live-test resource group/vnet/subnet and
`module.container_group` are planned/applied, then tear it down:

```bash
terraform destroy -var-file=config/container-groups.tfvars
```
...
terraform apply -var-file=config/container-groups.tfvars -var="pr_number=<pr-number>"

# 2. PR plan (and, in CI, apply), from A, against the same state file.
cd A/test/live
terraform init -backend-config="path=$STATE"
terraform plan -var-file=config/container-groups.tfvars -var="pr_number=<pr-number>"

# 3. Always tear down from A once the run finishes (`if: always()` in CI).
terraform destroy -var-file=config/container-groups.tfvars -var="pr_number=<pr-number>"
```
...
open PRs against this module never collide on the same sandbox resources.
